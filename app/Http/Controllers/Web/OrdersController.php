...
//            $text = "Dear ".$orders->customers_name.",\r\nYour order id: ".$orders_id.",\r\nTotal Amount: ".$orders->order_price;
//            UserInfo::smsAPI("88".$orders->delivery_phone,$text);
            //echo $payment_method . "okk";exit;
            //dd($payment_status);
            if($payment_status['payment_method'] == 'sslcommerz')
            {
                //dd("ksajdl");
                //echo 'okk';exit;

                //session(['order_id' => $orders_id]);
    ...
    public function getPaymentMethods()
    {
        ...
        /**   END INSTAMOJO**/
        /*************************/
        $payments_setting = $this->order->payments_setting_for_sslcommerz();
        //dd($payments_setting);
        $sslcommerz = array(
            'environment' => 1,
    ...
    {
        dd('sslcommerz order controller');
        $title = array('pageTitle' => Lang::get('website.Checkout'));
        $result = array();
